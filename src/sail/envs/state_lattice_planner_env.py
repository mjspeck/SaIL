...
from sail.network.supervised_regression_network import SupervisedRegressionNetwork
...
class StateLatticePlannerEnv(SearchBasedPlanner):
    def __init__(
        ...
        if lattice_type == "XY":
            ...
            self.start_n = self.lattice.state_to_node(
                (lattice_params["x_lims"][0], lattice_params["y_lims"][0])
            )
            self.goal_n = self.lattice.state_to_node(
                (lattice_params["x_lims"][1] - 1, lattice_params["y_lims"][0] - 1)
            )

        elif lattice_type == "XYH":
            self.lattice = XYHAnalyticLattice(lattice_params)
            self.start_n = self.lattice.state_to_node(
                (lattice_params["x_lims"][0], lattice_params["y_lims"][0], 0)
            )
            self.goal_n = self.lattice.state_to_node(
                (lattice_params["x_lims"][1] - 1, lattice_params["y_lims"][0] - 1, 0)
            )

        self.lattice.precalc_costs(
            self.cost_fn
        )  # Enumerate and cache successors and edge costs

        self.learner_policy = (
            None  # This will be set prior to running a polciy using set_learner_policy
        )

        # Data structures for planning
        self.frontier = (
            []
        )  # Frontier is un-sorted as it is sorted on demand (using heuristic)
        self.oracle_frontier = (
            PriorityQueue()
        )  # Frontier sorted according to oracle(for mixing)
        ...
        self.cost_so_far = defaultdict(
            lambda: np.inf
        )  # Keep track of cost of path to the node
        self.came_from = {}  # Keep track of parent during search

        self.learner = SupervisedRegressionNetwork(
            **learner_params
        )  # learner is a part of the environment
    ...
    def run_episode(self, k_tsteps=None, max_expansions=1000000):
        assert (
        ...
        self.cost_so_far[
            self.start_n
        ] = 0.0  # For each node, this is the cost of the shortest path to the start
        ...
        while len(self.frontier) > 0:
            ...
            if phase == "train":
                if curr_expansions in k_tsteps:
                    ...
                    data.append(
                        self.get_feature_vec[n], self.curr_oracle[n]
                    )  # Query oracle for Q-value of that action and append to dataset
            ...
            self.num_invalid_grand_children[
                self.came_from[curr_node][0]
            ] += n_invalid_edges
            ...
            for i, neighbor in enumerate(neighbors):
                ...
                if neighbor not in self.visited:
                    ...
                    if new_g < self.cost_so_far[neighbor]:
                        ...
                        self.learner.num_invalid_predecessors[neighbor] = (
                            self.num_invalid_predecessors[curr_node] + n_invalid_edges
                        )
                        self.learner.num_invalid_siblings[neighbor] = n_invalid_edges
                        self.learner.depth_so_far[neighbor] = (
                            self.depth_so_far[curr_node] + 1
                        )
        ...
        return (
            ...
            self.cost_so_far,
            self.c_obs,
        )  # Run planner on current env and return data seetn. Also, update current env to next env
    ...
    def sample_world(self, mode="cycle"):
        ...
        file_path = os.path.join(
            os.path.abspath(self.env_folder), str(self.curr_env_num) + ".png"
        )
        self.curr_env = initialize_env_from_file(file_path)

    def compute_oracle(self, mode="cycle"):
        file_path = os.path.join(
            os.path.abspath(self.oracle_folder),
            "oracle_" + str(self.curr_env_num) + ".p",
        )
