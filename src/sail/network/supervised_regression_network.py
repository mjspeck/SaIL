...
from planning_python.heuristic_functions.heuristic_function import (
    DubinsHeuristic,
    EuclideanHeuristicNoAng,
    ManhattanHeuristicNoAng,
)
...
class SupervisedRegressionNetwork:
    def __init__(
        ...
        self.display_step = 1
        self.summary_dir_train = os.path.join(
            os.path.abspath("saved_data/summaries"), summary_file + "_train"
        )
        self.summary_dir_test = os.path.join(
            os.path.abspath("saved_data/summaries"), summary_file + "_test"
        )
        ...
        if self.graph_type == "XY":
            self.dist_norm = (
                lattice_params["x_lims"][1] - lattice_params["x_lims"][0]
            ) * (
                lattice_params["y_lims"][1] - lattice_params["y_lims"][0]
            )  # Max possible distance between two nodes
            self.max_children = self.lattice.children.shape[0]
            self.coord_norm = np.array(
                [self.lattice.num_cells[0], self.lattice.num_cells[1]], dtype=np.float
            )
            ...
            self.coord_norm = np.array(
                [
                    self.lattice.num_cells[0],
                    self.lattice.num_cells[1],
        ...
        self.cost_so_far = defaultdict(
            lambda: np.inf
        )  # For each node, this is the cost of the shortest path to the start
        self.num_invalid_predecessors = defaultdict(
            lambda: -1
        )  # For each node, this is the number of invalid predecessor edges (including siblings of parent)
        self.num_invalid_children = defaultdict(
            lambda: -1
        )  # For each node, this is the number of invalid children edges
        self.num_invalid_siblings = defaultdict(
            lambda: -1
        )  # For each node, this is the number of invalid siblings edges (from best parent so far)
        self.num_invalid_grand_children = defaultdict(
            lambda: -1
        )  # For each node, this is the number of invalid grandchildren edges (seen so far)
        self.depth_so_far = defaultdict(
            lambda: -1
        )  # For each node, this is the depth of the node along the tree(along shortest path)
    ...
    def get_feature_vec(self, node):
        ...
        for h in self.heuristic_functions:
            feature_vec.append(
                h(node, self.goal_n) / self.dist_norm
            )  # Normalize the distances
        feature_vec.append(
            self.num_invalid_predecessors[node]
            / self.depth_so_far[node]
            * self.max_children
        )  # Normalized invalid predecessors
    ...
    def train(self, database):
        ...
        for epoch in range(self.training_epochs):
            ...
            for i in range(total_batch):
                ...
                _, c = self.sess.run(
                    [self.graph_ops["train_net"], self.graph_ops["cost"]],
                    feed_dict={
                        self.graph_ops["s"]: batch_x,
    ...
    def get_q_value(self, obs):
        ...
        output = self.graph_ops["output"].eval(
            session=self.sess, feed_dict={self.graph_ops["s"]: [obs]}
        )
        return output
    ...
    def set_params(self, input_params):
        [
            self.graph_ops["network_params"].assign(input_params[i])
